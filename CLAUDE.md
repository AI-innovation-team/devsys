...
- **发行:打 tag `v*` 触发 `.github/workflows/release.yml`** —— 出三个平台:**macOS 通用**(arm64+x86_64,`lipo`)、**Linux x86_64**(.deb/.AppImage/.rpm,用 ubuntu-22.04 编以放宽 glibc 兼容面)、**Windows x86_64**(.exe/.msi)。**改 workflow 先跑 `actionlint`** —— 曾被 `format('dev build #{0}', ...)` 里那个 `#` 坑过:空格后的 `#` 是 YAML 注释,把 `${{ }}` 截断,GitHub 在**校验阶段**就否掉整个文件(连 job 都不建,只留两个 0 秒失败 run)。Python 的 `yaml.safe_load` 查不出来,它只管 YAML 语法。先跑 `cargo test --lib` + 类型检查再打包。**每个平台都要先把 Go 的 `tsnet-helper` 编到 `tsnet-helper/tsnet-helper`**(`tauri.conf.json` 的 resources 指着它),macOS 要 `lipo` 成通用的,否则通用包在另一半架构上起不了 tailnet。`CGO_ENABLED=0`(tsnet 纯 Go,关掉才好交叉编译)。**`test` job 也必须编 helper** —— Tauri 的 build script 会校验 resource 存在,**连 `cargo test` 都过不去**(报 `resource path ... doesn't exist`)。**Windows 上文件名仍不带扩展名**(三平台共用同一条 resources 声明),`helper_path` 在运行时把它复制成 app 数据目录下的 `tsnet-helper.exe` 再起进程(Program Files 下的资源目录只读,不能就地改名)。
- **一个人可以有多台设备**(`member.devices: Vec<Device>`,不是单数):mac / 台式 / 工位机。**网络层早就装得下**(同一 GitHub 身份从三台机登录 = 三个 tailnet 节点),是 team.yaml 以前落后于它。节点名 = `<成员>-<设备名>`(`device_node_name`),设备名为空时退化成裸成员名 —— **保住老数据迁移后节点名不变**,否则 store 里的凭据和 `shared_to` 标记全失联。老格式的单数 `device:` 在 `parse_member` 里由 `normalize()` 折进 `devices`,只读不写。织物图上设备是**独立节点、挂在人旁边最内环**(不再折进人节点 —— 多台就折不下了)。
- **「设备 vs 服务器」由本人在共享面板勾**(`share_server` 的 `as_device`):设备**跟着人走**(合盖就没了),服务器是**基建**。这个区别只有本人分得清 —— 责任为门。改判时两边都会清干净,不留两份。
- **sidecar 的 stderr 绝不能丢**:`tailnet.rs` 曾用 `.stderr(Stdio::null())` —— 而 tsnet **接入失败几乎只在 stderr 说话**(头号坑「系统代理截断控制面 HTTPS」就是:状态一直停在 `starting`,stdout 一个字都没有)。用户只看到「一直在连接」,我们也无从判断。现已捕获 stderr、留最近 40 行、`diagnose()` 把已知症状翻成人话塞进 `status.hint`,UI 上 20 秒没连上就自动摊开日志。**新增任何 sidecar 都照此办理**。
- **`TailnetPanel` 必须拿到团队控制面地址**:设置页曾渲染成 `<TailnetPanel />`(没传)→ `control=""` → **连的是官方 Tailscale 而不是团队 Headscale**,而且不会给 helper 设 `NO_PROXY` → 有系统代理时必卡。现已改成传 `teamPath`、面板自己去读 `team.yaml` 的 `tailnet`。
- **构建期环境变量「设成空串」≠「没设」**:`option_env!` 对空串返回 `Some("")`。曾在 release workflow 里写 `DEVSYS_GH_CLIENT_ID: ${{ vars.XXX }}` 而仓库变量没配 → 展开成空串 → 烤进去的 client_id 是空的 → **分发版一打开就说「还没配 OAuth App」,用户根本没法登录**;而 dev 构建不设这个变量,本地永远复现不了。`ghauth::baked_client_id()` 已把空串当没设,回归测试 `baked_client_id_is_never_empty`。**凡是 `option_env!` 读的构建期变量都要这么防**。
- **三个平台都没有代码签名**:macOS 装完要 `xattr -dr com.apple.quarantine /Applications/DevSys.app`;Windows 的 SmartScreen 要「更多信息 → 仍要运行」。
